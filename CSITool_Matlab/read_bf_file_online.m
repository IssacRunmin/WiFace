...
%
function [ret, cursor] = read_bf_file_online(filename, start_point)
%% Input check
narginchk(2,2);
%% Open file and caculate the length of the file
f = fopen(filename, 'rb');
if (f < 0)
...
cur = start_point;              % Current offset into file
count = 0;                      % Number of records output
broken_perm = 0;                % Flag marking whether we've encountered a broken CSI yet
triangle = [1 3 6];             % What perm should sum to for 1,2,3 antennas

%% Process partly entries in file
